...
class ProductController {
  ...
  async getAllApplications(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch applications", error: error });
    }
  }
    ...
    try {
      ...
      if (products.length === 0) {
        res
          ...
          .json({
            success: false,
            message: "No products found for the specified adminId",
          });
        return;
      }
  ...
  async getJobApplicationByJobId(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (product) {
        res
          .status(200)
          .json({ message: "data fetched successfully", data: product });
      } else {
  ...
  async searchProducts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const products =
        await productServices.useSearchApplicationQuery(queryParams);
      res.json(products);
    } catch (error) {
  ...
  async getUniqueCompanyDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error retrieving unique company details" });
    }
  }
}
