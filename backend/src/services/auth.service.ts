  ...
  async verifyUserEmail(otpToken: string): Promise<ILoginResponse> {
    try {
      ...
      if (
        ...
        new Date() > new Date(userData.otpExpiration)
      ) {
        return {
      ...
      const token = jwt.sign(
        { id: doc.id, email: userData.email, role: userData.role },
      ...
      return {
        ...
        loading: false,
        token,
        user: { id: doc.id, ...userData } as unknown as IAuthUser,
      };
    } catch (error) {
