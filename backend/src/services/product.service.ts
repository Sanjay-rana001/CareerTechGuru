...
class ProductService {
    ...
    async getAllApplications(): Promise<IProduct[]> {
        try {
            ...
            return snapshot.docs.map(doc => ({ _id: doc.id, ...(doc.data() as any) })) as IProduct[];
        } catch (error) {
    ...
    async getJobApplicationsById(adminId: string): Promise<IProduct[]> {
        try {
            const snapshot = await db.collection('Product').where('adminId', '==', adminId).get();
            return snapshot.docs.map(doc => ({ _id: doc.id, ...(doc.data() as any) })) as IProduct[];
        } catch (error) {
            console.error('Error fetching products:', error);
            throw error;
    ...
    async useSearchApplicationQuery(queryParams : IQueryParams): Promise<IProduct[]> {
        try {
            ...
            return snapshot.docs.map((doc: any) => ({ _id: doc.id, ...(doc.data() as any) })) as IProduct[];
        } catch (error) {
