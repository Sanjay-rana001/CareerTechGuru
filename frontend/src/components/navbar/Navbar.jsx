    ...
    <>
      <div className='w-full sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm'>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
          ...
          <div onClick={handleNav} className='block md:hidden cursor-pointer text-slate-700 hover:text-slate-900 transition-colors'>
            {nav ? <AiOutlineClose size={24} /> : <AiOutlineMenu size={24} />}
          </div>

          {/* Mobile Navigation Menu */}
          {nav && (
            <div className="fixed inset-0 z-40 md:hidden bg-slate-900/20 backdrop-blur-sm" onClick={() => setNav(false)}>
              <ul
              ...
              >
                <li className="mb-4">
                  <Link to="/" onClick={() => setNav(false)} className="flex items-center gap-3 no-underline">
                    <img src={logoImg} alt="Logo" className="w-9 h-9 rounded-lg object-cover" />
                ...
                <li className="mt-auto pt-6 border-t border-slate-100">
                  {userRole ? (
                    ...
                    <button 
                      onClick={() => { navigate("/login"); setNav(false); }} 
