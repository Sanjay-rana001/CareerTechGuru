        ...
        return (
            <div className="min-h-[70vh] flex flex-col justify-center items-center bg-slate-50">
                <div className="bg-white border border-slate-200 p-8 rounded-2xl text-center shadow-sm max-w-md">
                    <h3 className="text-xl font-bold text-slate-800 mb-2">Job Not Found</h3>
                    <p className="text-slate-500 mb-6 text-sm">We couldn't retrieve the details for this job listing. It may have been closed or removed.</p>
                    <button onClick={() => navigate('/view-jobs')} className="px-6 py-2 bg-[#2563EB] hover:bg-blue-700 text-white font-medium rounded-lg border-0 transition-colors shadow-sm">
                        Back to Jobs
                    </button>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 mb-6">
                        ...
                        <div className="flex gap-4 items-start">
                            <div className="w-16 h-16 bg-slate-50 rounded-xl border border-slate-100 p-1 flex items-center justify-center overflow-hidden flex-shrink-0">
                                {jobData?.profilePicture ? (
                                    <img src={jobData.profilePicture} alt="Company Logo" className="max-h-full max-w-full object-contain" />
                                ) : (
                                    <span className="text-xs text-slate-300 font-bold uppercase">CTG</span>
                                )}
                            </div>
                            <div className="space-y-1">
                                <h1 className="text-2xl font-bold text-slate-900 leading-snug">{jobData?.title}</h1>
                                <p className="flex items-center gap-2 text-sm font-medium text-slate-500 mb-0">
                                    ...
                                    {jobData?.companyWebsite && (
                                        <a href={jobData.companyWebsite} className="text-slate-400 hover:text-[#2563EB] transition-colors" target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Experience</span>
                            <span className="text-sm font-semibold text-slate-700 capitalize">{jobData?.experience}</span>
                        </div>
                        <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Location</span>
                            <span className="text-sm font-semibold text-slate-700 capitalize">{jobData?.jobLocation}</span>
                        </div>
                        <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Salary Range</span>
                            <span className="text-sm font-semibold text-slate-700">{jobData?.salaryRange}</span>
                        </div>
                        <div>
                            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Min-Qualification</span>
                            <span className="text-sm font-semibold text-slate-700 capitalize">{jobData?.qualification}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-slate-50 rounded-xl p-4 border border-slate-100">
                        ...
                        <div>
                            {jobData?.isReferenceJob ? (
                                <button onClick={() => setConfirmBoxVisible(true)} className="px-6 py-2.5 bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-sm rounded-lg border-0 transition-colors shadow-sm">
